<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link href="data:image/png;base64," rel="icon" type="image/x-icon">
    <link rel="stylesheet" href="css/style.css?v=2">
    <script src="src/index.js"></script>
    <title>Mood Tracker</title>
</head>
<body>

<?php

@include 'mood-header.php';

?>

<section class="journals">

<h1 class="heading"><span>Journal</span> Searched</h1>

<div class="box-container">

<?php

if(isset($_POST['entry_date']))
{
    $entry_date= $_POST['entry_date'];
$in= mysqli_real_escape_string($connect, $entry_date);
$q= "SELECT userID, entry_date, entry_text FROM journal WHERE
    entry_date='$entry_date' ORDER BY userID";

	//run the query and assign it to the variable $result
	$result= @mysqli_query($connect, $q);
    if ($result)
	{
        while($fetch_journal=mysqli_fetch_array($result, MYSQLI_ASSOC))
        {
    ?>

<form action="" method="post">
   <div class="box">
      <h3><?php echo $fetch_journal['entry_date']; ?></h3>
      <div class="entry_text"><?php echo $fetch_journal['entry_text']; ?></div>
      <input type="hidden" name="entry_date" value="<?php echo $fetch_journal['entry_date']; ?>">
      <input type="hidden" name="entry_text" value="<?php echo $fetch_journal['entry_text']; ?>">
      <a href="journal-history.php?back=<?php?>" class="option-btn"> Go Back </a> </div>
   </div>
</form>

<?php
   };
};
}
?>

</body>
</html>